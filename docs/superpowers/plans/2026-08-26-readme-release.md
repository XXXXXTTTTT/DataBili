...
- [ ] Extract `word/media/image11.png`, `image12.png`, `image13.png`, `image17.png`, `image24.png`, and `image15.png` from `docs/社交网络大数据研究报告---B站数据挖掘与分析.docx`.
- [ ] Rename the extracted files according to the screenshot mapping in `docs/superpowers/specs/2026-08-26-readme-release-design.md`.
- [ ] Verify all six files exist and preserve their original PNG format.
- [ ] Commit the screenshot assets with the README release documents.
...
- [ ] Create an English document with the same functional sections and the same local screenshot paths as `README.md`.
- [ ] Preserve the exact risk boundary: no credential sharing, proxy evasion, fingerprint spoofing, CAPTCHA bypass, or other access-control bypass techniques.
...
- [ ] Run `git diff --check` limited to README and screenshot files.
- [ ] Confirm every referenced local image path exists.
- [ ] Commit only README, screenshot, specification, and plan files.
- [ ] Merge the documentation commits into local `main` without staging or modifying unrelated working-tree changes.
